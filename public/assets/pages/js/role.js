$(document).ready(function () {
    ...
    function getRole() {
        ...
        if ($('#roleTable').length > 0) {
            tableRole = $('#roleTable').DataTable({
                ...
                ajax: {
                    url: `/admin/role/getRole`, // Ganti dengan URL endpoint server Anda
    ...
    $("#formTambahRole").on("submit", function (event) {
        ...
        $.ajax({
            url: "/admin/role/storeRole", // Endpoint Laravel untuk menyimpan pegawai
            type: "POST",
            data: formData,
    ...
    $(document).on("click", ".btn-edit", function () {
        const roleIO = $(this).data("id");

        $.ajax({
            url: `/admin/role/getRoleByID/${roleIO}`, // Endpoint untuk mendapatkan data pegawai
            type: "GET",
            success: function (response) {
                // Ambil data pertama
    ...
    $(document).on("submit", "#formEditRole", function (e) {
        ...
        $.ajax({
            url: `/admin/role/updateRole/${idRole}`, // URL untuk mengupdate data pegawai
        ...
        const deleteID = $(this).data("id");
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`/admin/role/deleteRole/${deleteID}`, {
                        method: "DELETE",
                        headers: {
